...
public class TransitionUpdatesBroadcastReceiver extends BroadcastReceiver {
        ...
        if (intent != null) {
            if (ActivityTransitionResult.hasResult(intent)) {
                ...
                for (ActivityTransitionEvent event : result.getTransitionEvents()) {
                    ...
                    String activity = getActivityTypeString(event.getActivityType());
                    String transitionType = getTransitionTypeString(event.getTransitionType());
                    TransitionEntity transitionEntity = new TransitionEntity();
                    transitionEntity.setActivity_type(activity);
                    transitionEntity.setTransition_type(transitionType);
                    ...
                    OfflineEntries.getAppDatabase(context).transitionDao().insertDirectGrnOffline(transitionEntity);
                    Intent intent_transition_update = new Intent("Action_NEW_TRANSITION");
                    LocalBroadcastManager.getInstance(context).sendBroadcast(intent_transition_update);

                    //showNotificationOngoing(context, getActivityTypeString(event.getActivityType()) + ":" + getTransitionTypeString(event.getTransitionType()));
                    if (event.getTransitionType() == ActivityTransition.ACTIVITY_TRANSITION_ENTER) {
                        getLocation(event.getActivityType());
                    }
                }
            }
        }
